  ...
  <div>
    <el-form ref="myCardForm" v-model="myCardForm" label-position="right" label-width="120px">
      ...
      <el-form-item label="数据流文件:" required>
        ...
        <el-button type="primary" :disabled="isDisabled" @click="uploadBtnChecked">上传文件</el-button>
      </el-form-item>
      <el-form-item label="信任线:" required>
        <el-select v-model="myCardForm.trustLine" :disabled="isDisabled" placeholder="请选择信任线" style="margin-left:2px">
          <el-option v-for="item in trustLineState" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="板卡模式:" required>
        <el-select v-model="myCardForm.mode" disabled placeholder="请选择板卡模式" style="margin-left:2px">
          <el-option v-for="item in modeState" :key="item.label" :label="item.label" :value="item.label" />
        </el-select>
      </el-form-item>
    </el-form>
  ...
  methods: {
    setCardForm(cardForm) {
      this.myCardForm = {
        ...
        filenameLabel: '数据流文件',
        trustLine: cardForm.trustLine,
        mode: cardForm.mode
      }
      // if (this.myCardForm.trustLine === null) {
      //   this.myCardForm.trustLine = 1
      // }
      if (this.myCardForm.mode === null) {
        this.myCardForm.mode = 'EMD'
      }
