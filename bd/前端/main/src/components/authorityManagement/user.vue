  ...
  <section id="userManager" class="app-container">
    ...
    <el-col :span="24" class="query" style="padding-bottom: 0px;min-width:900px">
      <el-form ref="retrieveForm" :inline="true" :rules="retrieveRules" :model="retrieveForm" size="mini">
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" @click="retrieveUsers" />
        </el-form-item>
      </el-form>
    </el-col>
    ...
    >
      ...
      <el-table-column prop="userName" label="用户名" align="center" width="120px" :resizable="false" />
      <el-table-column prop="groundRoles" label="角色" align="center" :resizable="false" />
      <el-table-column prop="projects" label="项目" align="center" width="130px" :resizable="false" />
      <el-table-column prop="name" label="姓名" align="center" width="100px" :resizable="false" />
      <el-table-column prop="email" label="邮箱" align="center" :resizable="false" />
      <el-table-column prop="mobile" label="电话" align="center" :resizable="false" />
      <el-table-column prop="wechat" label="微信" align="center" :resizable="false" />
      <el-table-column prop="deptName" label="部门" align="center" width="100px" :resizable="false" />
      <el-table-column
        prop="createTime"
        label="创建时间"
    ...
    <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
      <el-form ref="addForm" :rules="addRules" :model="addForm" label-width="80px" style="margin-left:5%;">
        ...
        <el-form-item label="项目" prop="line" style="margin-top:20px">
      ...
      app.get('get_user', param).then(d => {
        if (d) {
          ...
          vm.users.forEach(item => {
            if (item.groundRoles) {
              item.groundRoles = item.groundRoles
                .map(itemI => itemI.roleName)
                .join(',')
            }
            if (item.projects) {
              item.projects = item.projects
                .map(itemI => itemI.name)
                .join(',')
            }
          })
        ...
        if (valid) {
          this.$confirm('确认提交吗？', '提交', {}).then(() => {
            ...
            const param = {
              ...
              projectIds: vm.$refs.tree.getCheckedKeys(true).join(',')
            }
        ...
        app.get('getUserById', param).then(d => {
          if (d) {
            vm.editForm = {
              ...
              groundRoleIds: d.msg.groundRoles[0].groundRoleId,
              checkedApprovalRoles: d.msg.approvalRoleNames
            }
            const projectIds = []
            const projects = d.msg.projects
            for (const project of projects) {
              projectIds.push(project.id)
            }
            vm.$refs.tree.setCheckedKeys(projectIds)
          }
        })
      }
    },
