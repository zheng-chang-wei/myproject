...
public class ReadExcelUtil {
	...
	private volatile static List<ThresholdData> list;

	public static List<ThresholdData> getThresholdDatas() throws Exception {
		if (list == null) {
			synchronized (ReadExcelUtil.class) {
				if (list == null) {
					...
					for (int i = firstRowNum + 1; i <= lastRowNum; i++) {
						...
						thresholdData.setTestNum(getCellValue(row.getCell(0)));
						thresholdData.setParameter1(getCellValue(row.getCell(1)));
						thresholdData.setParameter2(getCellValue(row.getCell(2)));
						thresholdData.setParameter3(getCellValue(row.getCell(3)));
