...
public class UserController {
    ...
    public ResponseBo getUser() {
        try {
            Map<String, Object> userMap = this.userService.findById(((User) SecurityUtils.getSubject().getPrincipal()).getId());
            User user = (User) userMap.get("tSysUser");
            User updateUser = new User();
            updateUser.setId(user.getId());
            if (user.getIsFirstLogin()) {
                updateUser.setIsFirstLogin(false);
                userService.updateNotNull(updateUser);
            }
