...
import com.hirain.qsy.shaft.dao.LogMapper;
...
public class LogServiceImpl extends BaseService<SysLog> implements LogService {
	...
	public void deleteLogsByTime(Date time) {
		...
		Criteria criteria = example.createCriteria();
		if (time != null) {
			criteria.andCondition("create_time<", time);
		}
		logMapper.deleteByExample(example);
	}

	@Override
	public void saveUploadFileLog(String originalFilename, String userName, String ip) {
		SysLog sysLog = new SysLog();
		sysLog.setOperation("上传文件");
		sysLog.setCreateTime(new Date());
		sysLog.setIp(ip);
		sysLog.setUsername(userName);
		sysLog.setParams(originalFilename);
		save(sysLog);
	}
}
