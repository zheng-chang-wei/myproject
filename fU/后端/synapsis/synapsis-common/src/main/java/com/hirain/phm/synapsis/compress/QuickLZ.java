...
public final class QuickLZ {
	...
	int headerLen(final byte[] source) {
		return (source[0] & 2) == 2 ? 9 : 3;
	}

	public long sizeDecompressed(final byte[] source) {
	...
	public long sizeCompressed(final byte[] source) {
		if (headerLen(source) == 9) {
	...
	private void write_header(final byte[] dst, final int level, final boolean compressible, final int size_compressed, final int size_decompressed) {
	...
	public byte[] compress(final byte[] source, final int level) {
		int src = 0;
		...
		byte[] d2;
		int fetch = 0;
		final int last_matchstart = source.length - UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END - 1;
		int lits = 0;
		...
		while (src <= last_matchstart) {
			...
			if (level == 1) {
				...
				if (cache == 0 && hash_counter[hash] != 0
						&& (src - o > MINOFFSET
								|| src == o + 1 && lits >= 3 && src > 3 && source[src] == source[src - 3] && source[src] == source[src - 2]
					...
					if (source[o + 3] != source[src + 3]) {
						...
						final int remaining = source.length - UNCOMPRESSED_END - src + 1 - 1 > 255 ? 255
				...
				int matchlen, k, m;
				byte c;
				final int remaining = source.length - UNCOMPRESSED_END - src + 1 - 1 > 255 ? 255 : source.length - UNCOMPRESSED_END - src + 1 - 1;
				int hash = (fetch >>> 12 ^ fetch) & HASH_VALUES - 1;
				...
				for (k = 0; k < QLZ_POINTERS_3 && (c > k || c < 0); k++) {
					o = hashtable[hash][k];
					if ((byte) fetch == source[o] && (byte) (fetch >>> 8) == source[o + 1] && (byte) (fetch >>> 16) == source[o + 2]
							&& o < src - MINOFFSET) {
						m = 3;
	...
	long fast_read(final byte[] a, final int i, final int numbytes) {
		long l = 0;
	...
	void fast_write(final byte[] a, final int i, final long value, final int numbytes) {
		for (int j = 0; j < numbytes; j++) {
			a[i + j] = (byte) (value >>> j * 8);
		}
	}

	public byte[] decompress(final byte[] source) {
		...
		final int last_matchstart = size - UNCONDITIONAL_MATCHLEN - UNCOMPRESSED_END - 1;
		int last_hashed = -1;
		int hash;
		int fetch = 0;
		...
		for (;;) {
			...
			if ((cword_val & 1) == 1) {
				...
				if (level == 1) {
					fetch = (int) fast_read(destination, last_hashed + 1, 3); // destination[last_hashed + 1] | (destination[last_hashed + 2] << 8) |
				...
				if (dst <= last_matchstart) {
					...
					if (level == 1) {
						while (last_hashed < dst - 3) {
							last_hashed++;
							final int fetch2 = (int) fast_read(destination, last_hashed, 3);
