...
public interface SimpleStatisticsMapper extends Marker {
	...
	class SimpleStatisticsSelectProvider {
		...
		public String countByMonth(StatisticsParm param) {
		...
		public String countByYear(StatisticsParm param) {
			String trains = getTrainString(param);
			StringBuilder sql = new StringBuilder();
			sql.append("select  r.year,r.month,sum(r.faults) fault,sum(r.subhealth) subhealth, sum(r.faults+r.subhealth) total from (\r\n");
			sql.append(
		...
		public String countByFaultType(StatisticsParm param) {
			String trains = getTrainString(param);
			StringBuilder sql = new StringBuilder();
		...
		private String getFaultCondition(StatisticsParm param, String trains) {
			StringBuilder sql = new StringBuilder();
		...
		public String countBySubHealthType(StatisticsParm param) {
			String trains = getTrainString(param);
			StringBuilder sql = new StringBuilder();
			...
			sql.append("where t1.subhealth_type_id=t2.id \r\n");
			String condition = getSubHealthCondition(param, trains);
			sql.append(condition);
			sql.append("group by t1.subhealth_type_id");

			return sql.toString();
		}

		private String getSubHealthCondition(StatisticsParm param, String trains) {
			StringBuilder sql = new StringBuilder();
			if (param.getStart_month() != null) {
		...
		public String countFault(StatisticsParm param) {
			StringBuilder sql = new StringBuilder();
		...
		public String countSubhealth(StatisticsParm param) {
			StringBuilder sql = new StringBuilder();
			sql.append("select count(*)\r\n");
